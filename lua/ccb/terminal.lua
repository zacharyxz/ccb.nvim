---Terminal provider for the ccb (Claude Code) CLI via Snacks.nvim.
...
---@field terminal_cmd string? Override the binary (default: "ccb")
...
---@param config ccb.TerminalConfig
  ...
  if config.auto_close then
    term_instance:on("TermClose", function()
      if vim.v.event.status ~= 0 then
        vim.notify(
          "ccb exited with code " .. vim.v.event.status,
          vim.log.levels.WARN
        )
      end
      terminal = nil
...
---@param user_config ccb.TerminalConfig?
...
local function get_cmd_and_env(cmd_args)
  local base = defaults.terminal_cmd or "ccb"
  local cmd
...
function M.open(opts_override, cmd_args)
  if not available() then
    vim.notify(
      "ccb.nvim: Snacks.nvim not available.",
  ...
  if term and term:buf_valid() then
    ...
    terminal = nil
    vim.notify(
      "ccb.nvim: Failed to open terminal.",
      vim.log.levels.ERROR
    )
  end
end
...
---Send text to the ccb terminal input.
---
...
function M.send_text(text)
  if not terminal or not terminal:buf_valid() then
    vim.notify(
      "ccb.nvim: No active terminal. Open one with :CCB first.",
      vim.log.levels.WARN
    )
    return
  end
